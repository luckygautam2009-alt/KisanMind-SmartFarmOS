...
import { Bot, X, Send, Mic, Sparkles, RefreshCw } from 'lucide-react';
...
export function AIAssistant() {
  ...
  useEffect(() => {
    setMessages([{ role: 'ai', text: t('aiWelcome') }]);
  }, [lang, t]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const hint =
        lang !== 'en'
          ? 'User may write in their selected UI language or English; reply in the same language when possible.'
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed bottom-24 right-6 w-80 sm:w-96 h-[500px] z-[120] glass-panel rounded-2xl flex flex-col shadow-2xl overflow-hidden border border-brand-200 dark:border-brand-500/20 bg-white/90 dark:bg-slate-900/95"
          >
            {/* Header */}
            <div className="p-4 bg-brand-500 dark:bg-brand-600 flex items-center justify-between text-white shadow-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm leading-tight">{t('aiChatTitle')}</h3>
                  <p className="text-[10px] text-brand-100 font-medium tracking-wide uppercase">{t('aiChatSub')}</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {sendLoading && (
                <div className="flex justify-start">
                  <div className="max-w-[80%] rounded-2xl p-3 text-sm bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-bl-sm flex items-center gap-2">
                    <RefreshCw className="w-4 h-4 animate-spin text-brand-500" />
                    <span className="text-slate-600 dark:text-slate-300">{t('plannerThinking')}</span>
                  </div>
                </div>
              )}
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-center gap-2">
              <div className="relative flex-1">
                <input 
                  ...
                  placeholder={lang === 'hi' ? t('chatInputPhHi') : t('chatInputPh')}
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-tr from-brand-600 to-brand-400 text-white rounded-full flex items-center justify-center shadow-lg shadow-brand-500/30 z-[120] group border border-white/20"
      >
